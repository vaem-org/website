    ...
    mounted() {
      ...
      if ($banner !== undefined) {
        if ($banner.dataset.src) {
          ...
          const width = _.find(sizes, function (x) {
            return (window.innerWidth * window.devicePixelRatio) <= x
          });

          img.src = width ? that.$options.filters.resize($banner.dataset.src,
            `${width}x0`) : $banner.dataset.src;
          img.onload = function () {
